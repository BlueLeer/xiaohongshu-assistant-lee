...
- 主要入口：`src/App.jsx`。这个文件包含 UI 状态、阶段流程、生成动作和用户主动触发逻辑。
- 本地 API 中间件：`vite.config.mjs` 注入 `server/codex/api.mjs`，处理浏览器对 `/api/*` 的请求。
- 本地 CLI 适配层：`server/localCli/registry.mjs` 负责 CLI 发现、`--version` 检测、命令校验、Codex/Kimi/Claude/自定义规范 CLI 调用，以及本地图片能力检查。
- 小红书搜索：`server/xhs/runXhs.mjs` 执行 `xhs` CLI，归一化搜索结果，并保持登录/未登录错误提示清晰。
- 生成与校验：`server/codex/prompts.mjs` 构建文本和封面 Prompt 任务指令，`server/codex/validation.mjs` 做输入校验与输出结构校验，`server/codex/coverImage.mjs` 管理封面图 prompt 和本地 PNG 发布。
- 设计与验收：`docs/SPEC.md` 是产品权威；`docs/DESIGN.md`、`docs/VERIFICATION.md`、`docs/QA_LOG.md` 记载视觉、交互、验证与 smoke test 要点。
- 不要：实现后台自动搜索、静默入库 RAG、自动发布/点赞/评论/收藏/关注、自动生成封面图；也不要在浏览器端直接执行 shell 命令。
- 若涉及封面 Prompt 或封面图生成，必须保持“明确排除真人、脸、手和动物”。
- 变更后请优先运行 `npm run build`，有 UI 变更时再运行 `npm run launch:fixed` 或 `npm run dev` 进行视觉检查。
